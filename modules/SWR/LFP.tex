...
\footnotetext{The electric field potential $\phi$ is only defined when the electric and magnetic fields do not vary too quickly over time, i.e. when $\dv*{\E}{t} \approx 0$ and $\dv*{\B}{t} \approx 0$. It then directly follows from Maxwell's equations that both fields become decoupled, and that the electric field becomes irrotational ($\curl{\E} = 0$), so that an electric potential $\phi$ may be defined as in \cref{eq:potential}. This so called \emph{quasi-static} assumption is met in electrophysiological conditions \cite{Nunez2006,Plonsey2007}.}
...
% % \\
% % \div{\grad{\phi}} &= \frac{1}{\sigma} \dv{\rho}{t} \\
