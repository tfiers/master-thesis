...
% The authors do not mention what type of IIR filter is used for filtering. Earlier in the supplemental methods section, when the recording is filtered between 600 and 6000 Hz to find spikes, they mention a ``2 pole Bessel for high and low pass''.
...
From the open source code of this study\footnote{\url{https://github.com/shayokdutta/RippleDetectionAnalysis/blob/master/DataAnalysisScripts/ripple_filtering.py}}, we find that the described 30-tap FIR filter (at $f_s = 3000$ Hz) is designed using the windowed-sinc method, with a Hamming window. We approximate this filter at a sampling frequency of 1000 Hz with a 11-tap FIR filter, designed with the same method, window type, and passband. The correspondence is good (see \cref{fig:dutta}).

\begin{figure}
\img{Dutta}
\captionn{Online band-pass filter from \authoryear{Dutta2018}}{}
\label{fig:dutta}
\end{figure}
