...
Extensions of simple rectification have also been used. In \citefull{Jadhav2012}, the online envelope $n_t$ is an exponentially weighted moving average (EWMA) of $\abs{o_t}$, with a variable gain. (In the update equation, a slightly higher weight is used for $\abs{o_t}$  whenever $n_t < \abs{o_t}$ -- see \cref{apx:online-detection-algos}). In \citefull{Dutta2018}, $\abs{o_t}$ is smoothed with a 50 Hz low-pass FIR filter.
