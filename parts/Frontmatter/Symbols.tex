...
\notation{p_t}{Transformation of $o_t$, constrained to $\reals^+$. Should be high when the corresponding input sample $\z_t$ is part of an SWR segment, and low when it is not. $\ p_t = \abs{o_t}$ for online linear filters; $p_t = \sigmoid(o_t)$ for the RNN's of \cref{ch:RNN}.}
